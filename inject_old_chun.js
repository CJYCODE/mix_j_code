...
async function getOrCreateTab(Target, port) {
    try {
        ...
        console.log(`Creating new window for port ${port} using CDP...`);
        const newClient = await CDP({ port });
        
        // Create a new window instead of a tab
        const { targetId: windowId } = await newClient.Target.createTarget({ 
            url: 'about:blank',
            newWindow: true
        });
        ...
        console.log(`Waiting for new window ${windowId} to be ready...`);
        const windowTarget = await waitForTargetReady(Target, windowId, 30);

        // Get all targets and find the first tab in our new window
        const { targetInfos: windowTargetInfos } = await Target.getTargets();
        const newTab = windowTargetInfos.find(t => 
            t.type === 'page' && 
            t.targetId !== windowId && // Exclude the window itself
            t.webSocketDebuggerUrl // Must have a debugger URL
        );

        if (!newTab) {
            throw new Error(`Could not find a tab in the new window ${windowId}`);
        }

        console.log(`Found new tab with ID: ${newTab.targetId}`);
